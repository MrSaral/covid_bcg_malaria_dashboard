function malaria() {
    ...
    var legend = svg.append("g").attr("class", "legend").attr("transform", "translate(50,450)");
    ...
    legend.append("text").text(function () { return "180+"; }).attr("transform", "translate(" + (legend_width - 20) + ",0)").style("font-size", "10px");
 ...
 function covid() {
     ...
     legend.append("text").text(function () { return (0 * 100).toFixed(1) + ""; }).attr("transform", "translate(0,0)").style("font-size", "10px");
     legend.append("text").text(function () { return "90,000+"; }).attr("transform", "translate(" + (legend_width - 20) + ",0)").style("font-size", "10px");
...
function bcg() {
    ...
    var colorScale = d3.scaleThreshold()
        .domain([0, 10, 25, 50, 75,85, 90,93, 95, 97, 99])
    ...
    d3.queue()
        .defer(d3.json, "https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson")
        .defer(d3.csv, "static/data/bcg_vaccine.csv", function (d) { data.set(d.Countries, +d.vac); })
        .await(ready);
        ...
        svg.append("g")
            ...
            .attr("fill", function (d) {
                ...
                tooltip.classed('hidden', false)
                    .attr('style', 'left:' + (mouse[0] + 10) +
                        'px; top:' + (mouse[1] + 150) + 'px')
                    .html(d.properties.name + " <br />" + (Math.round(data.get(d.properties.name) * 100) / 100 || 0));
            })
            .on('mouseout', function () {
                tooltip.classed('hidden', true);
            }).attr("class", "province");
    ...
    legend.append("text").text(function () { return (0 * 100).toFixed(1) + ""; }).attr("transform", "translate(0,0)").style("font-size", "10px");
    legend.append("text").text(function () { return "100"; }).attr("transform", "translate(" + (legend_width - 20) + ",0)").style("font-size", "10px");
